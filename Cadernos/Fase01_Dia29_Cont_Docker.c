/* ==============================================================
   Fase 01 — DIA 29 — }Continuação de Docker e Conteinerização
   ==============================================================
   ...
   COMPREENSÃO
   → Container é um processo isolado que só existe enquanto seu processo principal está em execução
   → Parar um container não o remove, permitindo sua reutilização, enquanto removê-lo apaga sua existência e seus dados
   → Os dados criados dentro do container ficam em sua camada de escrita e são perdidos quando o container é removido.
   → O comando COPY copia arquivos para a imagem durante o build, e alterações posteriores nos arquivos locais não afetam a imagem sem um novo build
   → O comando build constrói a imagem executando instruções reais, enquanto o run cria e executa um container a partir dessa imagem
   → Build só executa comandos reais, se não existe não irá criar a imagem
   → Docker utiliza cache de camadas para reutilizar partes já construídas, desde que instruções e contexto não tenham mudado.
   → Se uma camada for alterada, todas as camadas seguintes são reconstruídas.
   → A ordem das instruções no Dockerfile impacta diretamente a eficiência do cache e o tempo de build.
   ...
   - Próximo estudo: 1.4 Licenças e boas práticas de código aberto (MIT, Apache, GPL)
