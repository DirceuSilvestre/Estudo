/* ==============================================================
   Fase 01 — DIA 23 — Curso Prático de Bash
   ==============================================================
   ...
   --------------------------------------------------------------
    ...
    → $(( )) -- sintaxe do bash para operações aritiméticas
    → ${#string} -- Calcula o número de caracteres em uma string ${#"hello"}
    → $(expr index "$string" "$char") -- Encontra a posição de um caractere (índice inicia em 1) $(expr index "abcdef" "c") retorna 3
    → ${string:start:length} -- Extrai uma parte da string (índice inicia em 0) ${"hello":1:3} retorna ell
    → ${string/pattern/replacement} -- Substitui a primeira ocorrência de um padrão ${"hello"/l/L} retorna heLlo
    → ${string//pattern/replacement} -- Substitui todas as ocorrências de um padrão ${"hello"//l/L} retorna heLLo
    → ${string/#pattern/replacement} -- Substitui o padrão apenas se estiver no início da string ${"hello"/#he/HE} retorna HEllo
    → ${string/%pattern/replacement} -- Substitui o padrão apenas se estiver no final da string ${"hello"/%lo/LO} retorna helLO


   - Próximo estudo: Terminal Básico com Power Shell
